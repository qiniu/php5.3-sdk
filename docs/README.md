...
sdk提供了图片的水印处理方法。要对一个图片打水印，分为一下几个步骤：
首先对原图进行保护：
调用资源表对象 （`$rs`）提供的 `setProtected()` 方法，来设置对该资源表图像的保护。示例代码如下：
...
这样设置保护以后就没法通过 发布的域名+bucket+key 直接访问该图片了，从而实现了对原图的保护。


然后调用资源表对象 (`$rs`) 提供的 `setSeparator()`来设置分隔符。示例代码如下：
...
这样就设置了分隔符 `_` 这个分隔符你可以随意设置成您喜欢的格式。该分隔符的作用接下来您就知晓了。

最后来设置你图片的预览风格，示例代码如下：
	 ...
	 *设置图片的预览风格
	 */
	list($result, $code, $error) = $rs->setStyle("small", "imageView/0/w/64/h/64/	watermark/0");
	echo "===> set samll.jpg Style:\n";
	if ($code == 200) {
		echo "set samll Style success!\n";
	} else {
		$msg = QBox\ErrorMessage($code, $error);
		echo "set samll Style failed: $code - $msg\n";
	}
这样您就设置了名字为：`small` 的图片预览风格。这样您就可以通过以下url以您刚才设置的预览风格来访问已上传的图片： 发布的域名 + bucket + key + 分隔符（比如刚才设置的`_`） + 预览风格（比如刚才设置的`small`)其中预览风格的参数规格为：

	imageView/<mode>/w/<width/h/<height>/format/<format>	/q/<quality>/sharpen/<sharpen>/watermark/<wm>
其中<mode>设置为0表示水印往后是依次设设置图片的 宽度，高度，图片格式，图片质量，锐化度，以及是否设置水印。如果要设置水印接着往下看。

要设置水印首先要设置水印模板，示例代码如下：
	 ...
	 *设置默认的水印模板
	 */
	$wm_options = array("text"=>"defCustomer","bucket"=>"wmBucket", 'dx'=>20, 'dy'=>20);
	//这里set的customer字段设置为空，表示为默认模板。也可以指定客户id，给该客户设置水印模板
...
其中`$wm_options`为水印模板的参数，具体规格如下：

	$wm_options = array(
	    "customer" => <CustomerId>,
    	"gravity" => <GravityType>, =NorthWest, North, NorthEast, West, Center, East, 	SouthWest, South, SouthEast
    	"dissolve" => <ImageDissolve>,
    	"dx" => <DxInPixels>,
    	"dy" => <DyInPixels>,
	   	"fill" => <FontColor>,
    	"font" => <Font>,
    	"fontsize" => <FontSizeInTwips>,
    	"text" => <Text>
    	"bucket" => <ImageBucket>
	);
其中`customer`为您的客户的id，也就是说您可以给您的每个客户设置个性化的水印。当该值为空时，则为默认水印模板。`text`为要用来打水印的字符串，其中 图片用 \0 - \9 占位,表示`bucket`参数指定表中key 为0 - 9 所表示的图片。
如果水印模板设置成功，您就可以通过设置预览图风格时将watermark设置为非零数值来使用该默认模板了。如果想要获取已设置的模板参数可以通过：
	
	//这里是获取默认模板的参数，也可以指定`customer`字段来获取该客户的水印模板参数
...
如果请求成功，将会返回一个数组。包含有所有您设置的字段及设置的值。
...
在授权策略数组里传递customerId来标识该用户，当该用户设置有水印模板并且使用带有模板参数的图像预览风格来访问该上传的图片时，就会带上该用户设置的模板信息。
