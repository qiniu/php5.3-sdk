	...
	if (is_array($http_headers))
	{
        ...
        if (file_exists($tmphosts)) {
            if ($hoststr = file_get_contents($tmphosts)){
                $parsed["host"] = $hoststr;
                $curl_options[CURLOPT_URL] = buildURL($parsed);
            }
        }
	...
	curl_close($ch);
        ...
        if ($errno > 0) {
            ...
            if ($i < 3) {
                ...
                $parsed["host"] = $newhost;
                $newurl = buildURL($parsed);
                return ExecuteRequest($newurl, $parameters, $http_method, $http_headers, $form_content_type, $curl_extra_options, $i);
            }
        }
